...
import { Store, ACTIVE_CLUB_CHANGED_EVENT, type Club } from "@/lib/mcrStore";
...
export default function HomePage() {
  ...
  // Load clubs from Supabase so newly-created clubs appear immediately.
  useEffect(() => {
    if (!mounted) return;
    ...
    (async () => {
      try {
        ...
        // MVP: show clubs created by the signed-in user.
        // (Invites/memberships are implemented later.)
        ...
        if (!cancelled) setClubs((data ?? []) as any);
      } catch {
        if (!cancelled) setClubs([]);
      }
    })();
  ...
  const firstName = useMemo(() => {
    ...
    return full ? full.split(/\s+/)[0] : "Runner";
  }, [me]);
        ...
        {selectedClubId && clubName ? (
          ...
          <Card className="p-5">
            ...
            <p className="mt-1 text-[13px] text-black/55">
              Clubs unlock leaderboards, announcements, and a member directory. Joining requires an invite from a club admin.
            </p>
